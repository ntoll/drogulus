...
"""
Error = namedtuple('Error', ['uuid', 'code', 'title', 'details', 'version'])
...
uuid - the ping's request id (a random value generated by the requestor that
uniquely identifies the request).
version - the protocol version the message conforms to.
"""
Ping = namedtuple('Ping', ['uuid', 'version'])
...
version - the protocol version the message conforms to.
"""
Pong = namedtuple('Pong', ['uuid', 'version'])
...
timestamp - a timestamp indicating when this k/v was *originally* generated as
    a floating point number representing the time in seconds since the Epoch
    (so called POSIX time, see https://en.wikipedia.org/wiki/Unix_time).
...
'value', 'timestamp', 'name' and 'meta' fields. This mechanism ensures that the
public_key used in the compound key is valid (i.e. it validates the sig field
given the correct SHA1 hash) and also ensures that the 'value', 'timestamp',
'name' and 'meta' fields have not been tampered with.
...
version - the protocol version the message conforms to.
"""
FindNode = namedtuple('FindNode', ['uuid', 'key', 'version'])
...
version - the protocol version the message conforms to.
"""
Nodes = namedtuple('Nodes', ['uuid', 'nodes', 'version'])
...
version - the protocol version the message conforms to.
"""
FindValue = namedtuple('FindValue', ['uuid', 'key', 'version'])
